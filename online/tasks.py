...
def mws_task():
    ...
    for client in clients:
        api_data = client.apidata_set.filter(name="Amazon.de").first()

        if api_data is not None:
            access_key = decrypt_encrypted_string(api_data.access_key_encrypted)
            secret_key = decrypt_encrypted_string(api_data.secret_key_encrypted)
            account_id = decrypt_encrypted_string(api_data.account_id_encrypted)
            order_instance = OrderMws(access_key, secret_key, account_id, api_data)
            orders = order_instance.fetch_orders()
            print(f'Celery Task has run - Aufträge: {len(orders)}')
            for order in orders:
                print(f"---- {order}")
                order_status, order_id, order_products = order.get("OrderStatus"), order.get("AmazonOrderId"), \
                                                         order.get("products")
                purchase_date, earliest_ship_date = order.get("PurchaseDate"), order.get("EarliestShipDate")
                buyer_name = order.get("BuyerName")
                shipping_address = order.get("ShippingAddress")
